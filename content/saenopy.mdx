...
Challenges
----------
The original version of this 3D transaction force solver was written in a legacy C code base, which was not maintained
anymore and hard to install and use. I decided that it is crucial to modernize the code base, make it easy to install
and a pleasure to use.

First challenge was to delve into the complex C codebase with was not well commented and understand the mathematics
behind the method. Then I needed to rewrite the code in Python and achieve a similar performance heavily relying on
vectorisation with numpy and jit compilation with numba. Then the method needed to be improved to handle the boundary
conditions better to be able to work with very small image stacks for fast moving cells.
...
representing the fibers of the material. In contrast to most other solvers that only offer linear materials.

Heavy calculations are done using **Numpy** vectorization and
**Numba** jit-compilation. In contrast to other 3D traction force methods, saenopy does not rely on proprietary third
